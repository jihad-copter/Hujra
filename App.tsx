...
import { Book, Phone, MapPin, Search, PlusCircle, BrainCircuit, Users, History, ArrowLeft, GraduationCap, BarChart3, User, BookOpen, Trash2, AlertTriangle, Edit3, MessageSquare, Lightbulb, Download, Upload, Database, Settings, Share2, Info, X, Loader2, CheckCircle2, Copy, Check, ShieldCheck, Globe, Smartphone, Server, Github, Zap, UserPlus } from 'lucide-react';
        ...
        <div className="space-y-6">
          <div className="bg-emerald-900 rounded-[2.5rem] p-8 text-white kurdish-pattern shadow-xl relative overflow-hidden">
             <div className="relative z-10 text-right">
                <div className="flex items-center justify-end gap-2 mb-4">
                  <span className="bg-emerald-800 text-[10px] px-3 py-1 rounded-full flex items-center gap-1 border border-emerald-700 font-bold">
                    داتابەیس ناوخۆیی <ShieldCheck size={12} className="text-emerald-400" />
                  </span>
                </div>
                <h2 className="text-3xl font-black mb-1">بەڕێوەبەری حوجرە</h2>
                <p className="text-emerald-100 opacity-80 text-sm font-bold">سیستەمی تۆماری فەقێ و حوجرەکان</p>
                <div className="flex flex-wrap gap-2 mt-8">
                   <button onClick={() => setView('ADD_STUDENT')} className="bg-amber-500 text-emerald-950 px-6 py-3 rounded-2xl text-xs font-black flex items-center gap-2 active:scale-95 transition-all shadow-lg">
                     <UserPlus size={16} /> تۆمارکردنی فەقێی نوێ
                   </button>
                   <button onClick={() => setView('REPORTS')} className="bg-white/10 backdrop-blur-md text-white px-6 py-3 rounded-2xl text-xs font-black flex items-center gap-2 border border-white/20 active:scale-95 transition-all">
                     <BarChart3 size={16} /> ڕاپۆرتەکان
                   </button>
                </div>
             </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white p-7 rounded-[2.5rem] shadow-sm border border-emerald-50 text-center active:scale-95 transition-all cursor-pointer" onClick={() => setView('STUDENT_LIST')}>
               ...
               <p className="text-[10px] font-black text-gray-400 uppercase">کۆی فەقێکان</p>
            </div>
          ...
          <div className="bg-amber-50 p-6 rounded-[2.5rem] border border-amber-100 flex items-center justify-between">
             <div className="flex gap-2">
                <button onClick={exportData} title="Backup" className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-amber-600 shadow-sm border border-amber-100 active:scale-90 transition-all"><Download size={20}/></button>
                <label title="Restore" className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-emerald-600 shadow-sm border border-emerald-100 active:scale-90 transition-all cursor-pointer">
             ...
             <div className="text-right px-4 flex-1">
                ...
                <p className="text-[10px] text-amber-700 font-bold">هەناردە و هاوردەی زانیارییەکان</p>
             </div>
          ...
          <div className="space-y-4">
             <div className="flex justify-between items-center px-2">
               <h3 className="font-black text-gray-800">دوایین فەقێیەکان</h3>
               <button onClick={() => setView('STUDENT_LIST')} className="text-emerald-700 text-xs font-black">هەمووی</button>
             </div>
             {students.length === 0 ? (
                <div className="text-center py-10 bg-white rounded-[2rem] border border-dashed border-gray-200 text-gray-400 text-xs font-black">هیچ فەقێیەک تۆمار نەکراوە</div>
             ) : (
                students.slice(-3).reverse().map(s => (
                  <div key={s.id} onClick={() => { setSelectedStudentId(s.id); setView('STUDENT_DETAIL'); }} className="bg-white p-5 rounded-[2rem] shadow-sm border border-gray-50 flex items-center gap-4 active:scale-95 transition-all cursor-pointer">
                     <div className="w-14 h-14 bg-emerald-900 rounded-2xl flex items-center justify-center text-white font-black kurdish-pattern">
                       {s.photo ? <img src={s.photo} className="w-full h-full object-cover rounded-2xl" alt={s.fullName} /> : s.fullName[0]}
                     </div>
      ...
      {view === 'SETTINGS' && (
        <div className="space-y-6 text-right pb-10">
           <div className="flex items-center justify-between px-1">
              <h2 className="text-2xl font-black text-emerald-950">ڕێنمایی بڵاوکردنەوە</h2>
              <button onClick={() => setView('DASHBOARD')} className="text-emerald-700 font-black text-sm">داخستن</button>
           </div>
           
           <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-8">
              <section className="space-y-4">
                 <div className="flex items-center justify-end gap-3 text-blue-600 font-black">
                    ١. هەنگاوی یەکەم <Github size={20} />
                 </div>
                 <p className="text-sm text-gray-600 leading-relaxed font-bold">
                    کۆدی بەرنامەکە لە کۆمپیوتەرەکەت دابنێ و پاشان بەرزی بکەرەوە بۆ ناو 
                    <span className="text-gray-950"> GitHub </span> لە ئەکاونتێکی تایبەت.
                 </p>
              </section>
              ...
              <section className="space-y-4">
                 <div className="flex items-center justify-end gap-3 text-emerald-700 font-black">
                    ٢. هەنگاوی دووەم <Zap size={20} />
                 </div>
                 <div className="space-y-3 text-sm text-gray-600 font-bold">
                    <p>١. بڕۆ ناو <span className="text-blue-600">vercel.com</span> و پڕۆژەکەت Import بکە.</p>
                    <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100 text-amber-950">
                      <span className="font-black block mb-2 flex items-center justify-end gap-2 text-amber-800">گرنگ <AlertTriangle size={16}/></span>
                      کلیلی <span className="font-black text-red-700"> API_KEY </span> لە بەشی Environment Variables زیاد بکە بۆ ئەوەی AI کار بکات.
                    </div>
                 </div>
              </section>
              ...
              <section className="space-y-4">
                 <div className="flex items-center justify-end gap-3 text-purple-700 font-black">
                    ٣. هەنگاوی کۆتایی <Smartphone size={20} />
                 </div>
                 <p className="text-sm text-gray-600 leading-relaxed font-bold">
                    لینکە فەرمییەکەت لە Safari بکەرەوە و 
                    <span className="font-black text-gray-950"> Add to Home Screen </span> دابگرە بۆ ئەوەی وەک بەرنامە دابەزێت.
                 </p>
              </section>
           </div>
           ...
           <div className="bg-emerald-900 p-8 rounded-[3rem] text-white kurdish-pattern text-center space-y-4 shadow-xl">
              ...
              <p className="text-sm font-bold opacity-90 leading-relaxed">
                داتاکان تەنها لەناو موبایلەکەی خۆتدا دەمێننەوە. بۆ پاراستنیان، هەمیشە فایلی <b>Backup</b> دروست بکە.
              </p>
           </div>
        </div>
      )}
      ...
      {view === 'STUDENT_LIST' && (
        <div className="space-y-6">
          <div className="flex justify-between items-center px-1">
             ...
             <button onClick={() => setView('ADD_STUDENT')} className="bg-emerald-100 text-emerald-700 px-4 py-2 rounded-xl text-xs font-black flex items-center gap-1 active:scale-95 transition-all"><PlusCircle size={14}/> فەقێی نوێ</button>
          </div>
          ...
          <div className="space-y-3">
             {filteredStudents.length === 0 ? (
                <div className="text-center py-20 bg-white rounded-[2.5rem] border border-dashed border-gray-200 text-gray-300 font-bold">هیچ ئەنجامێک نەدۆزرایەوە</div>
             ) : (
      ...
      {view === 'VISIT_HISTORY' && (
        <div className="space-y-6">
          ...
          <div className="space-y-5">
            {visits.length === 0 ? (
               <div className="text-center py-20 bg-white rounded-[2.5rem] border border-dashed border-gray-200 text-gray-300 font-bold">هیچ سەردانێک تۆمار نەکراوە</div>
            ) : (
              visits.map(v => (
        ...
        <div className="space-y-6 text-right pb-10">
          ...
          <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-50 text-center space-y-6">
             <div className="relative inline-block">
               {selectedStudent.photo ? (
                 <img src={selectedStudent.photo} className="w-40 h-40 rounded-[2.5rem] object-cover border-4 border-white shadow-2xl" alt={selectedStudent.fullName} />
               ) : (
          ...
          {aiAnalysis && (
             <div className="bg-amber-50 p-8 rounded-[2.5rem] border border-amber-100 shadow-inner animate-in zoom-in-95 duration-300">
                <h4 className="font-black text-amber-900 mb-4 flex items-center justify-end gap-2 text-right">ڕاپۆرتی زیرەکی دەستکرد <BrainCircuit size={20}/></h4>
                <p className="text-base text-amber-950 leading-relaxed italic whitespace-pre-wrap font-bold text-right">{aiAnalysis}</p>
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-gray-50 space-y-8">
             ...
             <div className="space-y-4">
                ...
                {selectedStudent.currentBooks.length === 0 ? (
                   <p className="text-center py-4 text-gray-400 text-xs font-bold">هیچ کتێبێک تۆمار نەکراوە</p>
                ) : (
