...
import { Home, Users, UserPlus, History, BarChart3, Bell, WifiOff, Settings } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, currentView, onViewChange }) => {
  ...
  const navItems = [
    ...
    { id: 'SETTINGS' as ViewState, label: 'ڕێکخستن', icon: Settings },
  ];
    ...
    <div className="min-h-screen flex flex-col bg-[#fdfbf7] text-right pb-24 md:pb-0 md:pr-64">
      ...
      <header className="sticky top-0 z-40 bg-emerald-900 text-white p-4 shadow-md kurdish-pattern flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {!isOnline && (
              <div className="flex items-center text-[10px] text-amber-200">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-emerald-900 text-white p-6 shadow-xl h-screen fixed top-0 right-0 kurdish-pattern">
        <div className="mb-10 text-center border-b border-emerald-800 pb-6">
          ...
          <p className="text-emerald-200 text-sm mt-2">خزمەتی زانایانی ئایندە</p>
        </div>
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${
                currentView === item.id 
                ? 'bg-amber-500 text-emerald-950 shadow-lg scale-105 font-bold' 
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-gray-100 shadow-[0_-8px_30px_rgb(0,0,0,0.08)] flex justify-around items-center h-20 px-2 safe-area-bottom z-50">
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => onViewChange(item.id)}
          ...
          >
            ...
            <span className={`text-[9px] mt-1 font-bold ${currentView === item.id ? 'opacity-100' : 'opacity-70'}`}>
              {item.label}
            </span>
          </button>
        ))}
