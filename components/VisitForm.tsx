  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (addProgress) {
      const validStudyLogs = studyUpdates
        ...
        .map(u => ({
          bookName: u.bookName,
          currentPage: fromKu(u.page),
          teacherName: u.teacherName || formData.teacherName,
      ...
      if (financeUpdate.amount) {
        extraData.financeLog = { 
          amount: fromKu(financeUpdate.amount), 
             ...
             <div className="space-y-6 pt-2 animate-in fade-in zoom-in-95">
                  ...
                  <div key={update.id} className="p-5 bg-white rounded-3xl border border-emerald-100 shadow-sm space-y-4 relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       <div className="space-y-2">
                          ...
                          <input 
                            value={update.page} 
                            onChange={e => updateStudyItem(update.id, { page: fromKu(e.target.value) })} 
                            className="w-full bg-emerald-50/30 border border-emerald-100 rounded-xl p-3 text-right text-sm font-bold text-black shadow-inner" 
                            placeholder="١٤"
                          />
                       </div>
                ...
                <div className="grid grid-cols-2 gap-3 border-t border-emerald-100 pt-6">
                   ...
                   <div className="space-y-1">
                      ...
                      <input value={financeUpdate.amount} onChange={e => setFinanceUpdate({...financeUpdate, amount: fromKu(e.target.value)})} className="bg-amber-50/30 border border-amber-100 rounded-xl p-3 text-right text-sm font-bold text-black w-full" placeholder="٥٠٠٠" />
                   </div>
                </div>
             </div>
           )}
