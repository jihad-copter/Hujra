...
export class HujraDatabase {
  ...
  // Backup & Restore
  async exportAllData(): Promise<{students: Student[], visits: TeacherVisit[]}> {
    const students = await this.getAllStudents();
    const visits = await this.getAllVisits();
    return { students, visits };
  }

  async importAllData(data: {students: Student[], visits: TeacherVisit[]}): Promise<void> {
    const db = await this.open();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction(['students', 'visits'], 'readwrite');
      ...
      data.visits.forEach(v => visitStore.put(v));
